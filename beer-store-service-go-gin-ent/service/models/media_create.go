...
func (mc *MediaCreate) SetDatamedia(b []byte) *MediaCreate {
	mc.mutation.SetDatamedia(b)
	return mc
}

// SetNomemedia sets the "nomemedia" field.
func (mc *MediaCreate) SetNomemedia(s string) *MediaCreate {
	mc.mutation.SetNomemedia(s)
	return mc
}
...
func (mc *MediaCreate) check() error {
	...
	if _, ok := mc.mutation.Datamedia(); !ok {
		return &ValidationError{Name: "datamedia", err: errors.New(`models: missing required field "Media.datamedia"`)}
	}
	if _, ok := mc.mutation.Nomemedia(); !ok {
		return &ValidationError{Name: "nomemedia", err: errors.New(`models: missing required field "Media.nomemedia"`)}
	}
	if _, ok := mc.mutation.Mimemedia(); !ok {
		return &ValidationError{Name: "mimemedia", err: errors.New(`models: missing required field "Media.mimemedia"`)}
	}
	return nil
}
...
func (mc *MediaCreate) createSpec() (*Media, *sqlgraph.CreateSpec) {
	...
	if value, ok := mc.mutation.Creationdatemedia(); ok {
		...
		_node.Creationdatemedia = value
	}
	if value, ok := mc.mutation.Datamedia(); ok {
		_spec.Fields = append(_spec.Fields, &sqlgraph.FieldSpec{
			Type:   field.TypeBytes,
			Value:  value,
			Column: media.FieldDatamedia,
		})
		_node.Datamedia = value
	}
	if value, ok := mc.mutation.Nomemedia(); ok {
		_spec.Fields = append(_spec.Fields, &sqlgraph.FieldSpec{
			Type:   field.TypeString,
			Value:  value,
			Column: media.FieldNomemedia,
		})
		_node.Nomemedia = value
	}
	if value, ok := mc.mutation.Mimemedia(); ok {
		_spec.Fields = append(_spec.Fields, &sqlgraph.FieldSpec{
			Type:   field.TypeString,
			Value:  value,
			Column: media.FieldMimemedia,
		})
		_node.Mimemedia = value
	}
	return _node, _spec
}
